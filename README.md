...
- I have integrated these into the **Exome-Seq Secondary Analysis Pipeline** ([click here](https://github.com/manojmw/grexome-TIMC-Secondary)) which works at the `Clinical level`.
...
* It can be used on the `patient sample result file` generated by the [pipeline](https://github.com/manojmw/grexome-TIMC-Secondary).
...
- **Python version >= 3**
- External dependencies are kept to a minimum in all the scripts. The only required python modules are listed below: </br>
  * OpenPyXl == 3.0.10
  * SciPy == 1.5.2
